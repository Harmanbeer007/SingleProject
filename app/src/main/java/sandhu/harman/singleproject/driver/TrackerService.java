...
import sandhu.harman.singleproject.DriverFrag;
...
public class TrackerService extends Service implements LocationListener {
    ...
    private static final int CONFIG_CACHE_EXPIRY = 6000;  // 10 minutes.
    ...
    public void onCreate() {
        ...
        mPrefs = getSharedPreferences(getString(R.string.driverPref), MODE_PRIVATE);
        college = mPrefs.getString(getString(R.string.college_Id), "");

        String email = mPrefs.getString(getString(R.string.driverId), "");
    ...
    private void authenticate(final String email, String password) {
        ...
        mCollageInstance.addValueEventListener(new ValueEventListener() {
            public BusInfo value;
            public boolean driverListed = false;
            public String bus;

            @Override
            public void onDataChange(DataSnapshot dataSnapshot) {
                if (dataSnapshot != null && !dataSnapshot.toString().isEmpty()) {
                    for (DataSnapshot dataSnapshot1 : dataSnapshot.getChildren()) {

                        if (dataSnapshot1.getValue() != null && !dataSnapshot1.getValue().toString().isEmpty()) {
                            value = dataSnapshot1.getValue(BusInfo.class);

                            if (!driverListed) {
                                if (!value.getDriverId().isEmpty() && value.getDriverId() != null) {
                                    if (value.getDriverId().equals(email)) {
                                        driverListed = true;
                                        drivername = value.getDrivername();
                                        bus = value.getBusNumber();
                                    }
                                }
                            }
                    ...
                    if (driverListed) {
                        Toast.makeText(TrackerService.this, "Welcome " + drivername, Toast.LENGTH_LONG).show();

                        fetchRemoteConfig();
                        loadPreviousStatuses(bus);
                    } else {
                        Toast.makeText(TrackerService.this,
                                "Please Contact Your College to get Driver Id,Or Recheck the entered information.",
...
//                });
     ...
     */
    private void loadPreviousStatuses(String bus) {
        String transportId = mPrefs.getString(getString(R.string.college_Id), "");
        mFirebaseInstance = FirebaseDatabase.getInstance();
        mFirebaseTransportRef = mFirebaseInstance
                .getReference("ADMIN/" + mPrefs.getString(getString(R.string.college_Id), " ") + "/busLocation/" + bus);
...
//        mFirebaseTransportRef = FirebaseDatabase.getInstance().getReference(path);
    ...
    private GoogleApiClient.ConnectionCallbacks mLocationRequestCallback = new GoogleApiClient
        ...
        public void onConnected(Bundle bundle) {
            ...
            if (ActivityCompat.checkSelfPermission(TrackerService.this, android.Manifest.permission.ACCESS_FINE_LOCATION) != PackageManager.PERMISSION_GRANTED && ActivityCompat.checkSelfPermission(TrackerService.this, android.Manifest.permission.ACCESS_COARSE_LOCATION) != PackageManager.PERMISSION_GRANTED) {
    ...
    public void onLocationChanged(Location location) {
        ...
        transportStatus.put("driverId", mPrefs.getString(getString(R.string.driverId), " "));
        transportStatus.put("driverName", drivername);
    ...
    private void buildNotification() {
        ...
        PendingIntent resultPendingIntent = PendingIntent.getActivity(this, 0,
                new Intent(this, DriverFrag.class), PendingIntent.FLAG_UPDATE_CURRENT);
